...
import { formatTimestamp, openExplorer, safeMergeClassName, unitFormatter } from '@/utils/common';
...
import { tradeLogs } from '@/services/api/airdrop';
...
const TradingRewards = () => {
    ...
    const { data, loading } = useRequest(
        () => tradeLogs({ pageSize, page: current, creatorAddress: addressTokenStr || '' }),
        {
            ...
            <div className="border-x-[1px] border-solid border-[#2B3139] px-[1.5rem]">
                {loading ? (
                    ...
                    <DataTable
                        ...
                        stripedRows
                        pt={{
                            emptyMessage: { className: 'bg-[#181A20]' },
                            column: {
                                headerCell: () => ({
                                    className: safeMergeClassName(
                    ...
                    >
                        ...
                        <Column
                            ...
                            body={(record) => {
                                return <>{unitFormatter('sol', record.tradeVolFloat)} SOL</>;
                            }}
                        ></Column>
                        <Column
                            field="tx"
                            ...
                            body={(record) => {
                                return (
                                    <div
                                        onClick={() => {
                                            openExplorer(record.txHash, 'tx');
                                        }}
                                        className="w-[5.625rem] cursor-pointer truncate underline decoration-dashed"
                                    >
                                        {record.txHash}
                                    </div>
                                );
                        ...
                        ></Column>
                        <Column
                            field="tradingRewards"
                            header="Trading Rewards"
                            body={(record) => {
                                return <>+{record.total.toLocaleString()} $CRAZY</>;
                            }}
                        ></Column>
                    </DataTable>
                )}
